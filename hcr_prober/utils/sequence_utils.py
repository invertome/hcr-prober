...
def parse_mask_regions(region_str):
    ...
    for part in region_str.split(','):
        try: start, end = map(int, part.strip().split('-')); regions.append((start - 1, end))
        except ValueError: logger.warning(f'Could not parse masking region \'{part}\'.')
    return regions

IUPAC_MAP = {'R': ['A', 'G'], 'Y': ['C', 'T'], 'S': ['G', 'C'], 'W': ['A', 'T'], 'K': ['G', 'T'], 'M': ['A', 'C']}
def resolve_iupac_spacer(spacer_str):
    return ''.join([random.choice(IUPAC_MAP.get(c.upper(), [c])) for c in spacer_str])